...
func NewServiceStarter(vegaPaths paths.Paths, log *zap.Logger, svcStore *svcStoreV1.Store, walletStore walletapi.WalletStore, netStore walletapi.NetworkStore, connectionsManager *connections.Manager, interactor walletapi.Interactor, receptionChan <-chan interactor.Interaction) (*ServiceStarter, error) {
	s := &ServiceStarter{}
	s.reset()
	s.log = log
